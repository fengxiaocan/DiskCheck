...
public class FileUtils {
    ...
    private static boolean rename(File file, File newFile) {
        // 如果重命名的文件已存在返回false
    ...
    public static File renameFileType(File file, String type) {
        // 如果重命名的文件已存在返回false
        String extension = StringUtil.stringByDeletingPathExtension(file.getName());
        File newFile = new File(file.getParent(), extension + "." + type);
        rename(file, newFile);
        return newFile;
    }
    ...
    public static boolean writeFileFromIS(File file, InputStream is, boolean append) {
        ...
        try {
            ...
            StreamUtils.inputToOutput(is,os);
            return true;
        } catch (IOException e) {
            return false;
        } finally {
    ...
    public static String readFile2String(InputStream inputStream, String charsetName) {
        ...
        try {
            ...
            String line;
            while ((line = reader.readLine()) != null) {
                sb.append(line).append(Constants.SYS_SEPARATOR);
            }
            // delete the last line separator
            return sb.delete(sb.length() - Constants.SYS_SEPARATOR.length(), sb.length()).toString();
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        } finally {
            IoUtil.close(reader, inputStream);
        }
    }
    ...
    public static byte[] readFile2BytesByChannel(File file) {
        ...
        try {
            ...
            return StreamUtils.inputStream2Bytes(fis);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        } finally {
    ...
    public static String getBase64FromFile(File file) {
        ...
        try {
            ...
            base64 = Base64.getEncoder().encodeToString(StreamUtils.inputStream2Bytes(input));
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            IoUtil.close(input);
    ...
    public static String getBase64FromFile(String file) {
        return getBase64FromFile(new File(file));
    }
    ...
    public static byte[] getByte(File file) throws Exception {
        if (isFileExists(file)) {
            ...
            try {
                input = new FileInputStream(file);
                return StreamUtils.inputStream2Bytes(input);
            } finally {
                IoUtil.close(input);
            }
