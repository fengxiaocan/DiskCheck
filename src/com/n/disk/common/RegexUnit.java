...
public interface RegexUnit {
     ...
     * 正则:匹配URL中的img src
     */
     String REGEX_URL_IMASRC =
            "<img\\b[^>]*?\\bsrc[\\s]*=[\\s]*[\"']?[\\s]*(?<imgUrl>[^\"'>]*)[^>]*?/?[\\s]*>";

    /**
     * 正则:提取http
     */
     String REGEX_URL_SRC = "http[\\s]*[\"']?[\\s]*(?<imgUrl>[^\"'>]*)";

     String REGEX_HTTP_URL =
            "(https?|ftp|file)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]";

     String REGEX_IMG_SRC = "src=('|\")[^'\"]+('|\")";
    //提取href
     String REGEX_HREF = "href=('|\")[^'\"]+('|\")";
    /**
     * 提取html中加粗的文本
     */
     String REGEX_URL_STRONG = "<strong>(.|\\n)+</strong>";
